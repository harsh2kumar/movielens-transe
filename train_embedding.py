...
def train(args):
  ...
  dataloader = MovieLensDataLoader(args.batch_size)
  dataset=dataloader.load_edict()
  words_to_train = args.epochs * dataloader.total_samples + 1
  ...
  # print(dataloader.total_samples)
  # sys.exit(-1)
  pt=0
  for epoch in range(1, args.epochs + 1):
    ...
    while dataloader.has_next():
      ...
      lr = args.lr * max(1e-4, 1.0 - dataloader.finished_word_num_lr / float(words_to_train))
      for pg in optimizer.param_groups:
        pg['lr'] = lr

      # Get training batch.
      batch_idxs = dataloader.get_batch()
      # done this step inside KnowledgeEmbedding.compute_loss()
      ...
      if steps % args.steps_per_checkpoint == 0:
        ...
        logger.info('Epoch: {:02d} | '.format(epoch) +
                    'Words: {:d}/{:d} | '.format(dataloader.finished_word_num_lr, words_to_train) +
                    'Lr: {:.5f} | '.format(lr) +
    ...
    torch.save(model.state_dict(), '{}/embedding_1hop_des_epoch_{}.ckpt'.format(args.log_dir, epoch))
...
def main():
  ...
  args.dataset_dir = os.path.join(os.getcwd(), r'tmp/{}'.format(args.dataset))
  # args.dataset_file = args.dataset_dir + '/dataset.pkl'
  args.log_dir = args.dataset_dir + '/train_embedding'
