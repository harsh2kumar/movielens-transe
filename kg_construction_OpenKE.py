...
class MovieLens():
  ...
  def __init__(self):
    ...
    os.makedirs(r"data_processed/OpenKE", exist_ok=True)
    ...
    self.entity2id = r"data_processed/OpenKE/entity2id.txt"
    self.relation2id = r"data_processed/OpenKE/relation2id.txt"
    self.train2id = r"data_processed/OpenKE/train2id.txt"
    self.test2id = r"data_processed/OpenKE/test2id.txt"
...
"""
  ...
  def make_relations_OpenKE(self):
    """
create a new file for each qualified relation and store all possible interactions
:param indexes_only: when True, we write only indexs of entities and relations to file
:return:
"""
    ...
    self.make_entities()
    # relations with indices
    print('\nCreating relations with indices - for OpenKE (Step 1)\n')
    s0 = datetime.now()
    self.get_indices()
    dirs_to_explore = {self.relations_dir: self.train2id, self.test_dir: self.test2id}
    for dirs, indice_file in dirs_to_explore.items():
      count = 0
      for r, d, f in os.walk(dirs):
        for file in f:
          if '.txt' in file:
            ...
            print(file)
            with open(os.path.join(r, file), 'r') as f2r:
              with open(indice_file, 'a') as f2w:
                for lines in f2r:
                  line = lines.split()
                  head_entity = line[1].strip().split('.')[0]
                  tail_entity = line[1].strip().split('.')[2]
                  output = str(self.indices['entity'][head_entity][line[0].strip()]) + '\t' + \
                           str(self.indices['entity'][tail_entity][line[2].strip()]) + '\t' + \
                           str(self.indices['relation'][line[1].strip()]) + '\n'
                  f2w.write(output)
                  count+=1
            print('Processing completed. Time elapsed: ', datetime.now() - s)
...
"""
  ...
  def make_entity2id_relation2id(self):
    print('\nCreating entity2id mapping - for OpenKE (Step 2)\n')
    count=0
    with open(self.entity2id, 'w') as f2w:
      for entity_type, entity_dict in self.entity_indice_dict.items():
        print('\nProcessing entity_type: ', entity_type)
        # print(entity_dict)
        for entity, index in entity_dict.items():
         f2w.write(entity + " " + str(index) + "\n")
         count+=1

    with open(self.entity2id, 'r+') as f:
      content = f.read()
      f.seek(0, 0)
      f.write(str(count) + '\n' + content)

    count=0
    print('\nCreating relation2id mapping - for OpenKE (Step 3)\n')

    with open(self.relation2id, 'w') as f2w:
      for key, value in self.relation_indice_dict.items():
       f2w.write(key + " " + str(value) + "\n")
       count+=1

    with open(self.relation2id, 'r+') as f:
      content = f.read()
      f.seek(0, 0)
      f.write(str(count) + '\n' + content)
...
def main():
  ...
  # create relations
  sol.get_relations()
  sol.make_relations_OpenKE()
  sol.make_entity2id_relation2id()
