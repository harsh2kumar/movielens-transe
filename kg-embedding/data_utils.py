    ...
    def load_reviews(self):
        ...
        for line in self._load_file(self.review_file):
            ...
            for wi in word_indices:
                word_distrib[wi] += 1
            #wi, c = np.unique(word_indices, return_counts=True)
            #word_distrib[wi] += c
...
def main():
    ...
    for name in data:
        print('==', name, '==')
        dataset = AmazonDataset(data[name][0])
        if not os.path.isdir(data[name][1]):
            os.makedirs(data[name][1])
        with open(data[name][1] + '/dataset.pkl', 'wb') as f:
            pickle.dump(dataset, f)

    #with open(output_file, 'rb') as f:
    #    dataset = pickle.load(f)
    #dataloader = AmazonDataLoader(dataset, 8)
    #print(dataloader.get_batch())
