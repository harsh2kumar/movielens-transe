...
class KnowledgeEmbedding(nn.Module):
    ...
    def compute_loss(self, batch_idxs):
        ...
        rproduct1_idxs = batch_idxs[:, 5] # related product
        ...
        # product + mentions -> word
        pw_loss, pw_embeds = self.neg_loss('product', 'mentions', 'word', product_idxs, word_idxs)
    ...
    def neg_loss(self, entity_head, relation, entity_tail, entity_head_idxs, entity_tail_idxs):
        ...
        #et_empty_idx = self.entities[entity_tail].vocab_size
        #et_empty_idxs = torch.LongTensor(entity_tail_idxs.size()).fill_(et_empty_idx).to(self.device)
        #fixed_entity_tail_idxs = torch.where(entity_tail_idxs == -1, et_empty_idxs, entity_tail_idxs)
...
def kg_neg_loss(entity_head_embed, entity_tail_embed, entity_head_idxs, entity_tail_idxs,
    ...
    ########### Normalize ##############
    #entity_head_vec = F.normalize(entity_head_vec, p=2, dim=1)
    ####################################
    ...
    ########### Normalize ##############
    #entity_tail_vec = F.normalize(entity_tail_vec, p=2, dim=1)
    ####################################
    ...
    # Version 1
    #neg_sample_idx = torch.multinomial(distrib, batch_size * num_samples, replacement=True).view(batch_size, -1)
    #neg_vec = entity_tail_embed(neg_sample_idx)  # [batch_size, num_samples, embed_size]
    #neg_logits = torch.bmm(neg_vec, example_vec).squeeze() + relation_bias.unsqueeze(1)  # [batch_size, num_samples]
    #neg_loss = -neg_logits.neg().sigmoid().log().sum(1)  # [batch_size]

    # Version 2
    # for calculatng negative samples
...
def test_kg_neg_loss():
    entity_head_embed = nn.Embedding(5, 3)  # not sparse
    entity_tail_embed = nn.Embedding(8, 3)  # not sparse
    relation_vector = nn.Parameter(torch.FloatTensor([1, 2, 3]))
    entity_head_idx = torch.LongTensor([2, 4])
    entity_tail_idx = torch.LongTensor([3, 5])
    distrib = [2, 3, 4, 5, 6, 7, 8, 9]
    print(entity_head_embed.weight.size())
    print(entity_tail_embed.weight.size())
    print(relation_vector.size())
    print(entity_head_idx.size())
    print(entity_head_idx.size())
    loss = kg_neg_loss(entity_head_embed, entity_tail_embed, relation_vector, entity_head_idx, entity_tail_idx, 3,
                       distrib)
    print(loss)
...
if __name__ == '__main__':
    # test_kg_neg_loss()
    #output_file = './tmp/dataset.pkl'
    #with open(output_file, 'rb') as f:
    #    dataset = pickle.load(f)
    #dataloader = AmazonDataLoader(dataset, 8)
    #model = KnowledgeEmbedding(dataset, 100, 5)
    a = torch.FloatTensor([[1,1,1], [1,2,3]])
